        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                {loading ? <Skeleton className="h-4 w-20" /> : `${formatCurrency(stats?.totalProjectValue || 0)} total budget`}
              </p>
            </CardContent>
          </Card>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Your latest datacenter projects</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              {loading ? (
                Array.from({ length: 3 }).map((_, i) => (
                ...
                recentActivity?.projects.map((project) => (
                  <div key={project.id} className="flex items-center justify-between p-2 hover:bg-muted rounded-lg transition-colors">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-muted-foreground">{format(project.createdAt, 'MMM d')}</span>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      {project.totalBudget && (
                        <span className="text-sm font-semibold text-primary">{formatCurrency(project.totalBudget)}</span>
                      )}
              ...
              {!loading && (recentActivity?.projects.length || 0) > 0 && (
                <Button variant="outline" size="sm" className="w-full mt-3" onClick={() => navigate('/projects')}>
                  View All Projects
                  <ArrowRight className="ml-2 h-3 w-3" />
                </Button>
              )}
